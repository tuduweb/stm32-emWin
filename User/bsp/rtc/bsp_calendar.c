...
void GetChinaCalendarStr(uint16_t year,uint8_t month,uint8_t day,uint8_t *str)
{
	...
	StrCopy(&str[0],(u8 *)"甲子年正月初一",22);
	if(GetChinaCalendar(year,month,day,(u8 *)NLyear)==0)	return;
	GetSkyEarth(NLyear[0]*100+NLyear[1],&SEyear);
	StrCopy(&str[0],(u8 *)  sky[SEyear%10],3);	//  甲
	StrCopy(&str[3],(u8 *)earth[SEyear%12],3);	//  子	
	
	if(NLyear[2]==1)	StrCopy(&str[9],(u8 *)"正",3);
	else				StrCopy(&str[9],(u8 *)monthcode[NLyear[2]-1],3);		
	
	if(NLyear[3]>10) 	StrCopy(&str[15],(u8 *)nongliday[NLyear[3]/10],3);	
	else				StrCopy(&str[15],(u8 *)"初",3);
	StrCopy(&str[18],(u8 *)monthcode[(NLyear[3]-1)%10],3);
}
