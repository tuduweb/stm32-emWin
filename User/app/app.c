				...
				case GUI_ID_ICONVIEW1:
					switch (NCode) 
					{
						...
						case WM_NOTIFICATION_RELEASED: 
							...
							/* 打开相应选项 */
							switch(ICONVIEW_GetSel(pMsg->hWinSrc))//根据序号执行函数.
							{
								/* KEY  ******************************************************************/
								case 0:	
									Flag_ICON100 = 1;
			 ...
			 if (pInfo) 
			 {
				//printf("mooton_message=%d\n",pInfo->Cmd);
				 ...
				 {				    
					...
					case WM_MOTION_MOVE://移动的时候使组件失效
...
static void CreateTopWin(void)
{
	WM_HWIN hText;
	/* 顶部的 "wildfire OS "文本 */
	hText = TEXT_CreateEx(0, 2, 60 , 16, WM_HBKWIN, WM_CF_SHOW, GUI_TA_LEFT|TEXT_CF_VCENTER, GUI_ID_TEXT0, "GUI OS");
	TEXT_SetFont(hText, GUI_FONT_10_ASCII);
	TEXT_SetTextColor(hText,TEXT_COLOR);
	
	/* 状态栏的时间显示文本 */
	hText = TEXT_CreateEx(WinPara.xSizeLCD/2-25,2,50,16,WM_HBKWIN,WM_CF_SHOW,GUI_TA_HCENTER|TEXT_CF_VCENTER,GUI_ID_TEXT1,"00:00:00");
...
static void CreateMidWin(void)
{
	...
	WinPara.hWinMid = WM_CreateWindowAsChild(
								...
								WinPara.ySizeWin,	
								WM_HBKWIN, 
								WM_CF_SHOW | WM_CF_MOTION_X,//X轴可以应用MOTION效果的设置
								_cbMidWin, 
								sizeof(WIN_PARA *)
								);
/*-------------------------------------------------------------------------------------*/
...
*   Callback function for getting font data 获取字体信息
*
* Parameters:
